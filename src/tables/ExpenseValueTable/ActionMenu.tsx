...
const ActionsMenu: React.FC<{ detail: data }> = ({ detail }) => {
    ...
    MySwal.fire({
      ...
      html: (
        <div>
          ...
          <p> كود الايصال: {detail.receipt_code}</p>
          <p> قيمة المصروف: {detail.expense_value}</p>
          <p> الملاحظات: {detail.any_comments}</p>
        </div>
      ),
  ...
  const handleDelete = () => {
    ...
    fetch(
      `https://trombetta.mzservices.online/public/api/expense-values/${detail.id}`,
      {
        method: "DELETE",
        headers: {
